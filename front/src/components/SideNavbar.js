...
export default function SideNavbar() {
  ...
  const list = navElements.map((nav) => (
    <button
      className={`sm:p-2 sm:py-2 py-3 sm:pl-4 my-1 sm:text-left text-center sm:w-4/5 w-full font-karla tracking-wide font-bold text-gray-400 sm:text-base text-sm sm:mr-8 rounded-lg ease-in-out duration-300 ${
        nav.id === navID
          ? `bg-${nav.color}-600`
  ...
  return (
    <div className="w-screen flex sm:flex-row flex-col sm:justify-around sm:items-start items-center">
      <div className="sm:w-1/5 w-full flex flex-col sm:items-stretch items-center sm:mb-0 mb-2 px-2 sm:px-0">
      ...
      <div className="w-full sm:hidden sm:mb-0 mb-2 px-2">
        <div className="w-full bg-gray-700 rounded-lg py-1" />
      </div>
